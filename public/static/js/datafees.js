...
var datafeeds = function () {
   ...
   */
  function datafeeds(react) {
    console.log(react, "reactreactreactreactreactreactreactreactreactreact")
  ...
  datafeeds.prototype.getBars = function getBars(symbolInfo, resolution, periodParams, onDataCallback, onErrorCallback) {
    console.log("--------------------")
    var onLoadedCallback = function onLoadedCallback(data) {
      //if(data&&LastLength!=data.length){
      console.log(data, onDataCallback, "===========================================================onLoadedCallback", periodParams, "periodParams")
      if (data && data.length) {
        // 构建新的对象  
        const result = {
          t: [], // 时间数组  
          ...
          l: [], // 最低价数组  
          c: [], // 收盘价数组  
          v: [], // 交易量数组（注意：原始数据中没有交易量，这里留空或填充为null/0）  
          s: "ok" // 状态字符串  
        };
        data.forEach(item => {
          result.t.push(item.time);
          result.o.push(item.open);
          result.h.push(item.high);
          result.l.push(item.low);
          result.c.push(item.close);
          result.v.push(item.volume);
        });
        console.log(result, "result")
        onDataCallback(result, { noData: false });
      } else {
    ...
    this.self.getBars(symbolInfo, resolution, periodParams.from, periodParams.to, onLoadedCallback, onErrorCallback);
  };
