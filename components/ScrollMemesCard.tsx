...
export const ScrollMemesCard = (props) => {
  const router = useRouter();
  // 创建一个 ref 来引用可滚动的 div 元素  
  const scrollableDivRef = useRef(null);
  const userId = useLocalGlobalStore((state) => state.wallet.userId);
  const btnClassify = [{ name: "Trending" }, { name: "Hot" }, { name: "New" }];
  ...
  const LIMIT = 16; // 每页显示的项目数量  
  const apiUrl = "/token/list";

  const fetchData = async (page) => {
    setLoading(true);
    try {
      const data = await Api.post(apiUrl, { page: page, limit: LIMIT }, userId);
      if (!data.item) {
        setLoading(false);
        return;
      }
  ...
  const handlePageChange = (newPage) => {
    setPage(newPage);
    // 不需要在这里设置scrollTop，因为useEffect会处理
  };
  ...
  return (
    <div className="w-full flex flex-col gap-6 h-auto overscroll-none">
      <div className="flex flex-row w-full justify-between items-center" ref={scrollableDivRef}>
...
<div className="w-full gap-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 rounded-t-lg" >
      {items.map((item, index) => (
        <div key={index} className={`rounded-[14px] border-1 flex flex-row sm:flex-col bg-[#221f2e] ${item.badge == 4 ? "border-[#10F4B1]" : item.badge == 5 ? "border-[#FFC300]" : item.badge == 6 ? "border-[#FF3E80]" : "border-[#5a58f252]"} overflow-hidden  gap-0 hover:border-[#5a58f2] group/item relative`}>
          <Card
            shadow="sm"
            className="flex flex-row sm:flex-col h-[100%] max-w-[100%] w-[100%]  bg-[#221f2e] transition ease-in-out delay-50 hover:-translate-y-1 hover:scale-105 duration-200 sm:hover:h-[434.2px]"
            key={index}
            isPressable
          ...
          >
            <CardBody className="overflow-visible p-0 rounded-t-lg w-[45%] sm:w-full gap-0 shadow-none">
              ...
              <Image
                ...
                width="100%"
                radius="none"
                className="w-full object-cover rounded-t-lg h-[117px] sm:h-[274px] z-1 "
                src={item.image}
              />
            </CardBody>
            <CardFooter className="pb-0 pt-0 sm:pt-2 p-0 flex-col items-start justify-start text-left text-sm gap-0 w-full">
              <div className={`${items.length > 1 ? 'sm:group-hover/item:absolute sm:group-hover/item:bottom-8 sm:group-hover/item:left-0' : 'sm:group-hover/item:h-[152.2px]'} w-full`}>
                <div className=" flex flex-row text-small justify-between items-center gap-2 px-2 sm:px-4 w-full">
                  ...
                  <div className="flex flex-row justify-center items-center gap-1">
                    ...
                    <Image width="100%" src={item.avatar} className="w-[20px] h-[20px] sm:w-[24px] sm:h-[24px]" />
                    <Link
                      href={`/personalCenter?userId=${item.userId}`}
                      underline="always"
                      className="max-w-[30px] block whitespace-nowrap overflow-hidden text-ellipsis text-sm sm:text-base bg-transparent text-[#5a58fa] "
                    >
                      {item.createdBy}
                    </Link>
                    <p className="text-xxs sm:text-[14px] text-default-500 flex flex-row gap-1 justify-center items-center ml-2">
                      ...
                      <svg
                        ...
                        height="18"
                        viewBox="0 0 18 18"
                        className="w-[9] sm:w-[18] h-[9] sm:h-[18]"
                      >
                        <path
